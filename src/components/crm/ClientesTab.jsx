...
import { getClients, updateClientStatus, regenerateClientPassword, getAdministrativeAccessUrl, updateBusinessSettings } from '../../services/crmV3Service'
import { Users, ExternalLink, Key, RefreshCw, Lock, Shield, Layers, Package, MapPin, Copy, PauseCircle, PlayCircle, Archive, Edit3 } from 'lucide-react'
...
export default function ClientesTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#14161F] p-4 sm:p-6 rounded-2xl border border-white/5 shadow-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-400 mt-1">
            Restaurantes reales creados dentro de la plataforma StreetBoss.
          </p>
        </div>
          ...
          filteredClients.map(client => (
            ...
            >
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">
                      Propietario: <span className="text-gray-200 font-bold">{client.owner_name || 'N/A'}</span> · Tel/WA: <span className="text-emerald-400 font-mono">{client.whatsapp || client.phone}</span>
                    </p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                <div className="bg-[#0D0E12] p-3 rounded-xl border border-white/5 space-y-1">
                  <span className="text-gray-500 font-bold text-[10px] uppercase block">Menú Digital</span>
                  <a href={`/menu/${client.slug}`} target="_blank" rel="noreferrer" className="text-emerald-400 font-mono hover:underline truncate block">
                    /menu/{client.slug}
                  </a>
                </div>

                <div className="bg-[#0D0E12] p-3 rounded-xl border border-white/5 space-y-1">
                  <span className="text-gray-500 font-bold text-[10px] uppercase block">Usuario Propietario</span>
                  <span className="text-white font-mono truncate block">{client.owner_username || client.email || 'N/A'}</span>
                </div>

                <div className="bg-[#0D0E12] p-3 rounded-xl border border-white/5 space-y-1">
                  <span className="text-gray-500 font-bold text-[10px] uppercase block">Contraseña Temporal</span>
                  <span className="text-amber-400 font-mono truncate block">{client.temp_password || '********'}</span>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-white/5">
                <div className="flex flex-wrap items-center gap-2">
                  <a
                    href={`/menu/${client.slug}`}
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center gap-1.5 bg-white/5 hover:bg-white/10 text-white font-bold text-xs px-3 py-2 rounded-xl border border-white/5"
                  >
                    <ExternalLink size={13} /> Abrir menú
                  </a>

                  <a
                    href={getAdministrativeAccessUrl(client.slug)}
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center gap-1.5 bg-[#FF4B00]/10 hover:bg-[#FF4B00]/20 text-[#FF6A1A] font-bold text-xs px-3 py-2 rounded-xl border border-[#FF4B00]/20"
                  >
                    <Shield size={13} /> Acceso Administrativo al Dashboard
                  </a>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    ...
                    className="flex items-center gap-1 bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 font-bold text-xs px-3 py-2 rounded-xl border border-amber-500/20"
                  >
                    <RefreshCw size={13} /> Regenerar Contraseña
                  </button>
      ...
      {/* Modal Resultado Regenerar Contraseña */}
      {regenPassResult && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#14161F] border border-amber-500/30 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center gap-3 text-amber-400">
              <Key size={26} />
              <h3 className="text-lg font-black text-white">Contraseña Regenerada</h3>
            </div>
            <p className="text-xs text-gray-300">
              Se ha asignado una nueva contraseña temporal segura para <strong>{regenPassResult.name}</strong>.
            </p>
            <div className="bg-[#0D0E12] p-4 rounded-xl border border-white/10 space-y-2 text-xs font-mono">
              <p className="text-gray-400">Usuario: <span className="text-white">{regenPassResult.username}</span></p>
              <p className="text-gray-400">Nueva Contraseña: <span className="text-amber-400 font-bold text-sm">{regenPassResult.newPass}</span></p>
            </div>
            <div className="flex justify-end gap-3 pt-2">
              <button
                onClick={() => {
                  copyText(`Nueva Contraseña para ${regenPassResult.name}:\nUsuario: ${regenPassResult.username}\nContraseña: ${regenPassResult.newPass}`, 'regen')
                }}
                className="px-4 py-2 rounded-xl bg-[#FF4B00] hover:bg-[#FF6A1A] text-white text-xs font-black"
              >
                {copiedMsg === 'regen' ? '¡Copiado!' : 'Copiar Credenciales'}
              </button>
              <button
                onClick={() => setRegenPassResult(null)}
